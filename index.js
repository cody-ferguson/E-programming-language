...
var classmode = false, funcmode = false
file = args[0]
...
var tokens = [], newcontent = `from ${filePath}`, funcs = [], cfile = '',filen = 0
  lines.forEach((line) => {
  ...
  newlines.forEach((line,n) => {
    ...
    if (line.startsWith('from')) {
      ...
      if (line.endsWith(")")) {
        ...
        else if (/("?(?:\w*)"?) equals ("?(?:\w*)"?)/.test(content)) {
      ...
      if (/("?(?:\w*)"?) in ("?(?:\w*)"?)/.test(content)) {
        ...
        let matches = content.match(/("?(?:\w*)"?) equals ("?(?:\w*)"?)/)
